...
// === [TAMBAHAN] Util jaringan & explorer (tidak menghapus kode asli) ===
const NETWORK_INFO = {
  monadTestnet: {
    chainId: 10143,
    currency: "MON",
    explorerAddress: (addr) => `https://testnet.monadexplorer.com/address/${addr}`,
...
function getNetInfo(name) {
  return NETWORK_INFO[name] || NETWORK_INFO.default;
}
// === [TAMBAHAN] selesai ===
...
async function main() {
    ...
    // === [TAMBAHAN] Deteksi jaringan, validasi chainId khusus Monad ===
    const net = await ethers.provider.getNetwork();
    const info = getNetInfo(network.name);
    if (network.name === "monadTestnet" && net.chainId !== BigInt(info.chainId)) {
      console.warn(`⚠️  Expected chainId ${info.chainId} for Monad Testnet, but got ${net.chainId}. Periksa RPC/--network.`);
    }
    // === [TAMBAHAN] selesai ===
    ...
    // === [TAMBAHAN] Ganti label mata uang di log sesuai jaringan (MON untuk Monad)
    const currencyLabel = info.currency || "ETH";
    console.log(`   - Setting 1-Month price to ${ethers.formatEther(prices.oneMonth)} ${currencyLabel}...`);
    let setPriceTx = await subscriptionManager.setPriceForTier(0, prices.oneMonth);
    await setPriceTx.wait();
    console.log("     ✅ Done.");

    console.log(`   - Setting 6-Month price to ${ethers.formatEther(prices.sixMonths)} ${currencyLabel}...`);
    setPriceTx = await subscriptionManager.setPriceForTier(1, prices.sixMonths);
    await setPriceTx.wait();
    console.log("     ✅ Done.");

    console.log(`   - Setting 1-Year price to ${ethers.formatEther(prices.oneYear)} ${currencyLabel}...`);
    ...
    // === [TAMBAHAN] Cetak link explorer bila di Monad Testnet
    const addrLink = info.explorerAddress(diamondAddress);
    if (addrLink) {
      console.log(`🔎 Monad Explorer: ${addrLink}`);
    }
}
